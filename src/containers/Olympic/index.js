...
export default class Olympic extends React.Component {
	...
	initThree = () => {
		...
		var camera, scene, renderer, light, land = null,
			meshes = [],
			points = [];
		...
		function init() {
			...
			manager.onProgress = async (url, loaded, total) => {
				if (Math.floor(loaded / total * 100) === 100) {
					...
					_this.loadingProcessTimeout = setTimeout(() => {
						...
						Animations.animateCamera(camera, controls, {
							x: 0,
							y: -1,
							z: 20
						}, {
			...
			fiveCycles.map(item => {
				let cycleMesh = new THREE.Mesh(new THREE.TorusGeometry(100, 10, 10, 50), new THREE
					.MeshLambertMaterial({
			...
			fiveCyclesGroup.position.set(0, 10, -
				8);
			...
			for (let i = 0; i < 1500; i++) {
				let vertice = new THREE.Vector3(Math.random() * range - range / 2, Math.random() *
					range * 1.5, Math.random() * range - range / 2);
				// 纵向移速
	...
	render() {
		return ( <
			...
			this.state.loadingProcess === 100 ? '' : ( < div className = "olympic_loading" >
				<
				div className = "box" > {
